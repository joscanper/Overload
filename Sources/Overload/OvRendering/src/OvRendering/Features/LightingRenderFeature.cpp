...
{
	m_lightBuffer = std::make_unique<HAL::ShaderStorageBuffer>(Settings::EAccessSpecifier::STREAM_DRAW);
}
...
void OvRendering::Features::LightingRenderFeature::OnBeginFrame(const Data::FrameDescriptor& p_frameDescriptor)
{
	...
	m_lightBuffer->SendBlocks(lightMatrices.data(), lightMatrices.size() * sizeof(OvMaths::FMatrix4));
