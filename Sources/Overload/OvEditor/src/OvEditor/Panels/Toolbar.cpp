...
) : PanelWindow(p_title, p_opened, p_windowSettings)
{
	...
	m_playButton	= &CreateWidget<OvUI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Play")->GetTexture().GetID(), OvMaths::FVector2{20, 20});
	m_pauseButton	= &CreateWidget<OvUI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Pause")->GetTexture().GetID(), OvMaths::FVector2{ 20, 20 });
	m_stopButton	= &CreateWidget<OvUI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Stop")->GetTexture().GetID(), OvMaths::FVector2{ 20, 20 });
	m_nextButton	= &CreateWidget<OvUI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Next")->GetTexture().GetID(), OvMaths::FVector2{ 20, 20 });

	CreateWidget<OvUI::Widgets::Layout::Spacing>(0).lineBreak = false;
	auto& refreshButton	= CreateWidget<OvUI::Widgets::Buttons::ButtonImage>(EDITOR_CONTEXT(editorResources)->GetTexture("Button_Refresh")->GetTexture().GetID(), OvMaths::FVector2{ 20, 20 });
